...
Pod 起動を1本測った内訳です（2026-09-18 / RTX 4090 SECURE / `ACESTEP_XL_VARIANT=all`）。
text encoder が 0.6b + 4b の 2 本固定になった後の実測です。

| 段階 | 所要 |
|---|---|
| イメージ pull + 展開 | 3分19秒 |
| モデル DL | 37.1GiB を約1分14秒（約 515 MiB/s） |
| ComfyUI 起動 | 9秒 |
| 合計 | 4分42秒 |

`qwen_1.7b` を落とさなくなったので、`all` の DL は 40.5GiB から 37.1GiB に**減りました**。
同じ Pod を restart して測り直した 2 回目のモデル DL は 55 秒（約 691 MiB/s）で、
Hugging Face 側の振れ幅がそのまま出ます。

**モデルのダウンロードではなくイメージの取得が律速です**（全体の 71%）。そこを削ったのが
