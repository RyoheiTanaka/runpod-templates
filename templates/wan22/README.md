...
|---|---|---|
| `ComfyUI-Wan2.2-FreeCraftLog` | `runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04` | 安定版。まずはこちらを推奨。 |
| `ComfyUI-Wan2.2-cuda12.8-FreeCraftLog` | `runpod/pytorch:2.8.0-py3.11-cuda12.8.1-cudnn-devel-ubuntu22.04` | CUDA 12.8 対応 driver の host 向け。 |
...
```bash
bash -c 'RUNPOD_TEMPLATES_REF="${RUNPOD_TEMPLATES_REF:-v1.0.0}"; curl -fsSL "https://raw.githubusercontent.com/RyoheiTanaka/runpod-templates/${RUNPOD_TEMPLATES_REF}/templates/wan22/setup.sh" | bash'
```

`RUNPOD_TEMPLATES_REF=main` は常に最新の `main` を使います。公開 deploy link では再現性を重視し、`v1.0.0` などの固定 tag を使います。
...
| `RUNPOD_TEMPLATES_REF` | `v1.0.0` | setup script を取得する Git ref。公開運用では release tag、検証時は `main` など。 |
