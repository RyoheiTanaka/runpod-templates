...
まずは CUDA 12.4 image の `wan22` を使ってください。RunPod host driver との互換性が比較的高く、起動確認向けです。
...
| `wan22-cuda128` | `runpod/pytorch:2.8.0-py3.11-cuda12.8.1-cudnn-devel-ubuntu22.04` | CUDA 12.8 対応 driver の host 向け。 |
...
```bash
bash -c 'RUNPOD_TEMPLATES_REF="${RUNPOD_TEMPLATES_REF:-main}"; curl -fsSL "https://raw.githubusercontent.com/RyoheiTanaka/runpod-templates/${RUNPOD_TEMPLATES_REF}/templates/wan22/setup.sh" | bash'
```

`RUNPOD_TEMPLATES_REF=main` は常に最新の `main` を使います。再現性を重視する場合は `v1.0.0` などの固定 tag を推奨します。
...
| `RUNPOD_TEMPLATES_REF` | `main` | setup script を取得する Git ref。`main`, `v1.0.0`, commit SHA など。 |
